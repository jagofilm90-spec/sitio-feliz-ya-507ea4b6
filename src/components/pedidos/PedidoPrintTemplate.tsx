...
export const PedidoPrintTemplate = ({ datos, hideQR = false, variante }: Props) => {
  ...
  return (
    <div className="p-5 bg-white text-black w-[11in] min-h-[8.5in] mx-auto font-sans flex flex-col" style={{ fontSize: "11px" }}>
        ...
        <div className="w-32 flex-shrink-0 text-right">
          {isAlm && datos.pedidoId && !hideQR ? (
            <div className="flex flex-col items-center">
              ...
              <p className="font-black mt-1" style={{ fontSize: "11px", color: "#C8102E" }}>{datos.folio}</p>
            </div>
          ) : (
            <div>
      ...
      {/* ══ BARRA RESUMEN ══ */}
      <div className={`grid ${showPrices ? "grid-cols-3" : "grid-cols-2"} gap-0 border border-gray-400 rounded mb-2`} style={{ fontSize: "11px" }}>
        {showPrices && (
        ...
        <div className={`${showPrices ? "border-r border-gray-300" : ""} px-3 py-1.5`}>
      ...
      {showPrices && (
        <div className="flex justify-between items-start mb-2 gap-4">
          <div className="flex-1">{datos.notas && <div className="border border-gray-300 rounded p-2" style={{ fontSize: "10px" }}><p className="font-bold uppercase text-gray-500 mb-0.5" style={{ fontSize: "9px" }}>Notas</p><p>{datos.notas}</p></div>}</div>
      ...
      {isAlm && <div className="flex justify-end mb-2"><div className="bg-gray-100 border border-gray-400 rounded px-4 py-2 text-right"><span className="font-bold uppercase text-gray-500" style={{ fontSize: "10px" }}>Peso Total: </span><span className="font-bold" style={{ fontSize: "16px" }}>{kgFmt(datos.pesoTotalKg)}</span></div></div>}

      {/* ══ AVISO + PAGARÉ (original) ══ */}
      {isOrig && (
        <div className="grid grid-cols-2 gap-2 mb-2" style={{ fontSize: "9px" }}>
          ...
          <div className="border border-gray-400 rounded p-2">
            <p className="font-bold uppercase mb-1" style={{ fontSize: "10px" }}>Pagaré</p>
      ...
      {/* ══ FIRMAS (original) ══ */}
      {isOrig && (
        <div className="grid grid-cols-3 gap-6 mb-2 mt-1">
          <div className="text-center pt-6 border-t border-black"><p className="text-gray-500" style={{ fontSize: "9px" }}>Nombre y firma</p></div>
          <div className="text-center pt-6 border-t border-black"><p className="text-gray-500" style={{ fontSize: "9px" }}>Fecha de recepción</p></div>
          <div className="text-center pt-6 border-t border-black"><p className="text-gray-500" style={{ fontSize: "9px" }}>Sello del cliente</p></div>
        </div>
      )}
