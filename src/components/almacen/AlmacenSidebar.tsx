...
export const AlmacenSidebar = ({
  ...
  const renderNavItem = (item: NavItem) => (
    <button
      ...
      className={cn(
        "w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all min-h-[52px]",
        "hover:bg-white/10",
        activeTab === item.id
    ...
    >
      ...
      {item.badge !== undefined && item.badge > 0 && (
        <Badge 
          ...
          className="h-6 min-w-6 flex items-center justify-center text-xs"
        >
    ...
    <aside className="hidden lg:flex flex-col w-64 bg-slate-900 fixed left-0 top-0 h-screen z-40">
      ...
      <div className="p-4 border-b border-slate-700">
        <div className="flex items-center gap-3">
          <img src={logoBlanco} alt="Almasa" className="h-10 w-auto" />
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">Sincronizado</span>
            </div>
          </div>
        </div>
      </div>
