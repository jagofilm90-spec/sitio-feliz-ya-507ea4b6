...
CREATE OR REPLACE FUNCTION aplicar_credit_hold(p_cliente_id uuid, p_motivo text) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION liberar_credit_hold(p_cliente_id uuid, p_motivo text DEFAULT 'Liberación manual') RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION cambiar_credito_limite(p_cliente_id uuid, p_nuevo_limite numeric, p_motivo text) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
