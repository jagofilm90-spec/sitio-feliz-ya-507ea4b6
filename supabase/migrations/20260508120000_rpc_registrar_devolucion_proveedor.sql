...
-- Fix Bug #1A: devoluciones a proveedor no decrementaban inventario
-- Transacción atómica: INSERT devolución + decremento lote + movimiento auditoría
-- Patrón: registrar_baja_caducidad
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Validar que producto coincide con el lote
  ...
  -- Obtener nombre producto para notificación
  ...
  -- 2. Decrementar lote (permite negativo)
  UPDATE inventario_lotes
  SET cantidad_disponible = cantidad_disponible - p_cantidad,
      updated_at = now()
  WHERE id = p_lote_id;

  -- Obtener stock post-decremento
  ...
  -- 4. Si stock negativo, notificar admin
